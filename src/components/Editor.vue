...
import '../../node_modules/rangy/lib/rangy-classapplier.js';
export default {
  name: 'Editor',
  data () {
    return {
  ...
  methods: {
    init: function () {
           ...
           this.sourceClassApplier = rangy.createClassApplier('data-source', {
                   ...
                   onElementCreate: (el) => {
                     var id = 'source-' + this.uuid();
                     el.id = id;
                     this.saveSource(id);
                     el.addEventListener('click', () => {
                       this.selectText(id);
                       this.getToolbarElement().classList.add('source-options');
           ...
           this.factClassApplier = rangy.createClassApplier('data-fact', {
                   ...
                   elementAttributes: {'data-disable-editing': true},
                   onElementCreate: (el) => {
                     var id = 'fact-' + this.uuid();
                     el.id = id;
                     this.saveFact(id);
                     el.addEventListener('click', () => {
                       this.selectText(id);
                       this.getToolbarElement().classList.add('fact-options');
                       this.showAndUpdateToolbar();
                       var fact = this.facts.filter((fact) => {
                         return fact.id === id;
                       });
                     });
                   }
       ...
       createToolbarButtons: function () {
           ...
           sourceButton.addEventListener('click', () => {
             this.sourceClassApplier.applyToSelection();
             this.hideToolbar();
           })

           factButton.addEventListener('click', () => {
             this.factClassApplier.applyToSelection();
             this.hideToolbar();
           })

           removeButton.addEventListener('click', () => {
             var el = this.getSelectionParentElement();
             if(el.className !== 'data-fact' && el.className !== 'data-source') return false;

             if(el.className === 'data-fact') {
                this.removeFact(el.id)
              } else if (el.className === 'data-source') {
                this.removeSource(el.id)
              }
           ...
           selectSourceButton.addEventListener('click', () => {
             var factId = this.getSelectionParentElement().id;
             var sourceList = document.querySelectorAll('.data-source');
             var fact = this.facts.filter((fact) => {return fact.id == factId;})[0];
             ...
             var detectSource = function (factId, event) {
               ...
               setTimeout(this.hideToolbar(), 300)
           ...
           showFactsButton.addEventListener('click', () => {
             var sourceId = this.getSelectionParentElement().id;
             this.facts.forEach((fact) => {
      ...
      saveSource: function(id) {
        setTimeout(() => {
          var text = document.getElementById(id).innerText;
          this.sources.push({id: id, text: text})
          console.log(this.sources)
        }, 1000)
      },

      saveFact: function(id) {
        setTimeout(() => {
          var text = document.getElementById(id).innerText;
          this.facts.push({id: id, text: text})
          console.log(this.facts)
        }, 1000)
      },

      removeFact: function(id) {
        setTimeout(() => {
          this.facts = this.facts.filter((el) => { return el.id != id; });
          console.log(this.facts)
        }, 1000)
      },
      removeSource: function(id) {
        setTimeout(() => {
          this.sources = this.sources.filter((el) => { return el.id != id; });
          console.log(this.sources)
        }, 1000)
      }
